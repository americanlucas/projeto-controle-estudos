...
export default function Menu() {
    ...
    function MenuColapsado() {
        return(
            <>
                <main className={corFundo? "menu bg-zinc-600 w-30 h-screen": "menu bg-zinc-600 w-30 h-screen"} >
                    <div className="flex flex-row justify-center mb-12">
                        <button onClick={()=>setColapsaMenu(!colapsaMenu)} className="botao"><MenuIcon size={30}/></button>
                    </div>
                    <div className="flex justify-center flex-1">
                        {colapsaItem? <div className="duration-200 ease-in">
                            <div className="flex flex-1 py-2 px-3 flex-col gap-3 justify-center">
                                <div className="flex hover:bg-zinc-500 p-2 rounded-md text-lg justify-center cursor-pointer">
                                    <a href="/contador"><BookCopy size={30}/></a>
                                    <div className="hidden">
                                        <MenuItem nomeLink='Contador' url="/contador"/>
                                    </div>
                                </div>
                                <div className="flex hover:bg-zinc-500 p-2 rounded-md text-lg justify-center cursor-pointer">
                                    <a href="/ranking"><ChartNoAxesCombined size={30}/></a>
                                    <div className="hidden">
                    ...
                    <Link href={`/perfil`} className="flex justify-center">
                        <img className="rounded-full" src="https://i.pinimg.com/736x/5c/9d/2a/5c9d2a13557b7330b3e91cf9a7670d1a.jpg" alt="Foto de perfil lucas" width={50} height={20}/>
                    </Link>
                </main>
    ...
    return (
        ...
        <section className="duration-400 ease-in-out">
            ...
            <main className={`${corFundo ? "menu":"menu"} ${colapsaMenu? "w-70": "w-30"}`} >
                <div className="flex flex-row justify-between border-b-2 border-zinc-500 px-4 ">
                    <h1 className="pb-5 text-2xl font-bold">Menu</h1>
                    <button onClick={()=>setColapsaMenu(!colapsaMenu)} className="botao"><MenuIcon/></button>
                </div>
                <div className="flex flex-col flex-1 mb-4">
                    <div onClick={()=>setColapsaItem(!colapsaItem)} className="flex items-baseline justify-between p-2 cursor-pointer hover:text-zinc-300 duration-200">
